...
fn test_substitution_funs() {
    ...
    let f = FuncDecl::new(&ctx, "f", &[&Sort::int(&ctx), &Sort::int(&ctx)], &Sort::int(&ctx));
    let x = ast::Int::new_const(&ctx, "x");
    let y = ast::Int::new_const(&ctx, "y");
    let sub = f.apply(&[&x, &y]).as_int().unwrap();

    let a = ast::Int::new_var(&ctx, 0);
    let b = ast::Int::new_var(&ctx, 1);
    let x_plus_y = &a + &b;
    let test = sub.substitute_funs(&[(&f, &x_plus_y)]);

    let real = ast::Int::add(&ctx, &[&x, &y]);
